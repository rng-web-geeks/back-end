...
使用过Java的同学都使用过方法调用，大部分同学平常可以主要专注在使用，对于方法是选择定位以及运行期怎么调用可能都没有太关心，实际上Java方法调用有不同的表达方式，有大家熟悉的方式，也有不太常见的方式，还有一整套解析和执行规则，简单说，它包括两个阶段，一是编译期的方法匹配，二是执行期的方法确定和执行，本篇文章咱们先来一块探究下编译期怎么确定被调用的方法。

## Java方法调用表达式
Java的方法调用有好几种不同的表达方式，有我们熟悉的方式也有一些不常见的调用方式，要理解编译器怎么确定方法调用，需要先掌握不同的调用方式，因为不同的方式会影响到方法的解析。
按照Java语言规范的定义，以下是主要的Java方法调用表达式的定义格式：

MethodInvocation(方法调用):
* MethodName( [ArgumentList] ), 方法名带上调用参数，这是最常见的方法表达式，可以是实例方法，也可以是静态方法。 
* TypeName.[TypeArguments] Identifier ( [ArgumentList] )，静态方法调用的常见形式。
* ExpressionName.[TypeArguments] Identifier ( [ArgumentList] )
* Primary.[TypeArguments] Identifier ( [ArgumentList] )，这种方式和前一种方式类似，被调用的方法取决于前面的表达解析结果，它可能是类静态方法调用或者实例方法调用。
...
## Java方法调用编译期类型确定
Java的方法在Java世间是二等公民，必须依附于某个类才能存在，所以编译器首先需要先确定从哪个类(或者接口)作为入口点来查找目标方法。
按照Java语言规范的定义，总共有6种不同的情况，这里我们逐一来分析：
 1. 如果调用形式是MethodName，(也就是上一节方法调用表达式的第一种形式)，Java编译器执行一种称为“comb rule”的查找规则，具体就是：
    ...
    * 如果上面两种都不存在，最后会检查是否有static import的方法和MethodName一致。<br/>
    简单说，就是先从自己的类继承结构查找，接着往嵌套它的外部类继承结构查找(如果有嵌套类的情况），最后看是否有static import的情况。**注意这个阶段它只关心方法名是否匹配，并不关心方法签名等其他因素，匹配到方法名就结束搜索**过程。
 ...
 5. super. [TypeArguments] Identifier, 常见的调用父类方法，查找入口是父类的继承结构。
...
## 案例分析
光说不练假把式，掌握了理论是为了更好的应用，下面我们来一起分析几个典型案例。
...
```
按照上面的规则，不执行装箱和拆箱操作进行匹配，“20”可以安全的转换为long类型，所以会匹配到“calc(long value)”这个方法，执行的时候会打印出“20 calc by calc(long value).”, 执行结果有点反直觉。
