...
* 同年荷兰CA提供商DigiNotar服务器被伊朗黑客攻破，黑客签发了包括Goolge，Facebook，美国中情局和军情六处等在内的531个网站证书，并用于窃听伊朗用户的Gmail信息等，该事件直接导致该公司在一年后破产，有兴趣可以查看这个[新闻链接](HTTPS://cnzhx.net/blog/diginotar-ca-servers-been-compromised/)；
...
目前很多大公司包括Google和Facebook都开始监控自己的域名，Google通过监控发现赛门铁克在Google不知情的情况下提交了一张预签发证书，如果你的网站是金融或电子商务网站，强烈建议把自己的域名加入监控。
...
本来只想写一个短文普及下证书的知识，没想到越写越多，关于互联网的安全，给人的感觉是为了解决一个问题，然后引入更多的问题，道高一尺魔高一丈。

后记：网上有很多以讹传讹的文章，说证书包含公钥和私钥，这是极大的谬误，如果真有人把公钥和私钥都包含在证书，了解了RSA算法原理后，就会知道那将是非常可笑的一件事，实际上确实有同时包含公钥和私钥的文件，那是PKCS #12格式的文件，后缀名是".p12"或".pfx",它是CA机构颁发证书的时候，
同时生成公钥和私钥，签名完打包成".p12"格式发回给申请人，申请人可以用openssl解析出证书和私钥文件，这种方式实际上有一定的安全性问题，至少CA机构知道你的私钥，虽然它们号称会及时销毁私钥，另外EV证书不允许用这种方式申请，".p12"和证书实际上是两码事。
